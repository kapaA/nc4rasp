...
int main(int argc, char *argv[])
{
	...
	bool multiple_relay = false;
	bool relay_overhear = false;

    po::options_description desc("Allowed options");
    desc.add_options()
	    ("relay_list", po::value<>(&relay_list)->multitoken(), "List of relays.")
	    ("multiple_relay_enabled", po::value<bool>(&multiple_relay), "Multiple relays is enabled")
        ...
        ("relay_overhear", po::value<bool>(&relay_overhear), "enable overhearing at relay");
	...
	if (link_quality == true)
	{
        ...
        else if (type == "destination")
        {
            if (field == "binary")
            {
                ...
                receive<rlnc_decoder>(port, iteration, symbols, symbol_size, e1, e2, e3, output, id, ovear_estimate, sourceID, helperID, helper_flag, syntetic_loss, relay_list, multiple_relay);
            }
            else if (field == "binary8")
            {
        ...
        else 
        {
            ...
            {
                typedef kodo::full_rlnc_decoder<fifi::binary> rlnc_decoder;
                auto r = new relay<rlnc_decoder>(host,port, rate, iteration, symbols, symbol_size, max_tx, e1, e2, e3, syntetic_loss, output, id, strategy, ovear_estimate, sourceID, relayID, DestinationID, helperID, helper_flag, credit_app, relay_overhear);
                r->forward();
            }
            else if (field == "binary8")
            {
